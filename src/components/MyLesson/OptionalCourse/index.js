...
export default class OptionalCourse extends PureComponent {
    ...
    cancelPay = (item, e) => {
        ...
        this.props
            .dispatch({
                type: 'studentDetail/cancelPay',
                payload: {
                    studentChooseCourseId: item.studentChooseCourseId,
                },
            })
            .then(() => {
    ...
    checkToMessage = () => {
        let hash =
            ...
            window.location.hash.split('#/') &&
            window.location.hash.split('#/')[1];
        let currentUrl = `${window.location.origin}${window.location.pathname}?fromDetail=${hash}#/course/student/PersonalMessage`;
    ...
    readyPay = (item, e) => {
        e.stopPropagation();
        if (item.orderStatus == 1 && item.tuitionOrderNo && item.tuitionPlan) {
            let iframeUrlMan =
                window.location.origin.indexOf('yungu.org') > -1
    ...
    checkDetai = (chooseCoursePlanId, coursePlanningId, item) => {
        ...
        //     return;
        // } else {
        window.location.href = `${window.location.origin}${window.location.pathname}#/courseDetail/${coursePlanningId}/${chooseCoursePlanId}/false`;
        // }
                    ...
                    {optionalCourseList && optionalCourseList.length > 0 ? (
                        optionalCourseList.map((item, index) => {
                            return this.props.type === 1 ? (
                                <div
                                    className={[styles.listItem, styles.noPadding].join(' ')}
                                    key={index}
                                ...
                                >
                                    <span className={styles.header}>
                                        ...
                                        <div className={styles.myLessonContent}>
                                            <div
                                                style={{
                                                    backgroundImage: item.courseCover ? `url("${item.courseCover}")` : `url('https://yungu-public.oss-cn-hangzhou.aliyuncs.com/learn/list/lessondefault.png')`,
                                        ...
                                        <div className={styles.stuNameBox}>
                                            {/* <div className={styles.stuName}>
                                                <div><i className={icon.iconfont}>&#xe60f;</i>{item.userName || ''}</div>
                                            </div> */}
                                            ...
                                            <div className={styles.rightLesson}>
                                                {item.tuitionOrderNo &&
                                                item.orderStatus ==
                                                    1 ? null : item.chooseStudentStatus == 0 ? (
                                                    <div className={styles.noSuccess}>
                                                        {trans(
                                                            'course.basedetail.no.selected',
                                                            '未选中'
                                                        )}
                                                    </div>
                                                ) : item.chooseStudentStatus == 1 ? (
                                                    item.type == 1 ? (
                                                        <div className={styles.paySuccess}>
                                                            {trans(
                                                                'course.basedetail.selected',
                                                                '已选中'
                                                            )}
                                                        </div>
                                                    ) : (
                                                        <div className={styles.paySuccess}>
                                                            {trans(
                                                                'mobile.successfulRegistration',
                                                                '报名成功'
                                                            )}
                                                        </div>
                                                    )
                                                ) : item.chooseStudentStatus == 3 ? (
                                                    <div className={styles.paySuccess}>
                                                        {trans('course.header.submitted', '已提交')}
                                                    </div>
                                                ) : item.chooseStudentStatus == 4 ? (
                                                    <div className={styles.time}>
                                                        {trans('mobile.redeyCancle', '已取消')}
                                                    </div>
                                                ) : null}
                                            </div>
                                        </div>
                                        <div
                                            className={[styles.stuNameBox, styles.statusBox].join(
                                                ' '
                                            )}
                                        >
                                            {item.timeRemaining ? (
                                                <div className={styles.time}>
                                                    {lang == 'en' ? (
                                                        <span>
                                                            ...
                                                            {item.timeRemaining % 60} remaining
                                                        </span>
                                                    ) : (
                                                        <span>
                                                            {trans('global.remainder', '还剩')}
                                                            {Math.floor(item.timeRemaining / 60)}
                                                            {trans('global.min')}
                                            ...
                                            <div className={styles.rightLesson}>
                                                {item.cancelSignUp ? (
                                                    <div
                                                        ...
                                                        onClick={this.cancelPay.bind(this, item)}
                                                    >
                                                        {trans('pay.cancelPay', '取消报名')}
                                                    </div>
                                                ) : null}
                                                {item.tuitionOrderNo && item.orderStatus == 1 ? (
                                                    <div
                                                        className={styles.readyPay}
                                                        onClick={this.readyPay.bind(this, item)}
                                                    >
                                                        {trans('pay.readytoPay', '立即支付')}
                                                    </div>
                                                ) : null}
                                                {/* {item.chooseStudentStatus == 4 ? (
                                                    <div className={styles.time}>
                        ...
                        <div className={styles.noData}>
                            ...
                            <p className={styles.text}>
                                {trans('mobile.mentionMyLesson', '暂无已报课程，请返回首页选报')}
                            </p>
                        </div>
                    )}
                </div>
