...
export default class PayDetail extends PureComponent {
    ...
    goList = () => {
        ...
        if (previewContent && previewContent.tuitionType == 1) {
            ...
            if (device == 'dingtalk') {
                ...
                let iframeUrlMan =
                    window.location.origin.indexOf('yungu.org') > -1
                        ? `https://smart-scheduling.yungu.org/?/myCourse#/wdMobilePay/index`
                        : `https://smart-scheduling.daily.yungu-inc.org/?/myCourse#/wdMobilePay/index`;
                window.location.href = iframeUrlMan;
            } else {
                let iframeUrlMan =
                    window.location.origin.indexOf('yungu.org') > -1
        ...
        if (useBalance && payNumber > 0) {
            ...
            if (previewContent.arrearsOfMoney && parseFloat(previewContent.arrearsOfMoney) > 0) {
                dispatch({
                    ...
                    const { payList, previewContent } = this.props;
                    console.log(previewContent.submitType, 'previewContent');
                    let fundChannel = '';
                    let payType = 1;
                    let data =
                        payList &&
            ...
            return;
        } else if (useBalance && payNumber <= 0) {
            dispatch({
                type: 'mobilePay/useBalance',
                payload: {
                    deductionAmount:
    ...
    confirmPay = () => {
        ...
        console.log('previewContent.arrearsOfMoney', previewContent.arrearsOfMoney);
        console.log('previewContent.userWalletAmount', previewContent.userWalletAmount);
        //getUrlSearch('tuitionType')
        dispatch(
            routerRedux.push({
                pathname: '/confirmPay/index',
                search: `tuitionPlan=${getUrlSearch(
                    'tuitionPlan'
                )}&fundChannel=${fundChannel}&device=${device}&payType=${payType}&previewContent=${
    ...
    render() {
        ...
        statusType = previewContent.payStatus;
        console.log('statusType', statusType);
        console.log('previewContent', previewContent);

        console.log('previewContent.payStatus', previewContent.payStatus == 'true');
                    ...
                    <div className={styles.head}>
                        ...
                        {previewContent &&
                            (previewContent.payStatus ? (
                                ...
                                <span className={styles.head}>
                                    {trans('pay.paying', '正在支付中')}
                                </span>
                            ) : previewContent.payOrderStatus == 5 ? (
                                <span className={styles.head}>{trans('pay.close', '已关闭')}</span>
                            ) : (
