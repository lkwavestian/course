...
import { intoChinese, getCourseColor, fixedZero, formatTimeSafari } from '../../../../utils/utils';
...
export default class MobileWeekTable extends PureComponent {
    ...
    renderCol(number, map, currentTime) {
        let cols = [],
            curD = (currentTime ? new Date(formatTimeSafari(currentTime)) : new Date()).getDay(),
    ...
    //处理时间
    handleTime(val) {
        let time = new Date(formatTimeSafari(val));
    ...
    //处理时长
    timeLong(start, end) {
        var startStr = new Date(formatTimeSafari(start));
        var endStr = new Date(formatTimeSafari(end));
